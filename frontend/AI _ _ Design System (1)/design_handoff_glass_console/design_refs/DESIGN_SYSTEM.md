...
The system was originally drafted in a **warm parchment** direction (cream paper + navy ink + signal orange). After exploring three alternatives (`explorations/glass`, `cyber`, `editorial`), the team selected **light glass** as the canonical direction. The parchment variant is preserved unchanged for reference:

- `colors_and_type-parchment.css` — original parchment tokens
- `ui_kits/console-parchment/` — full parchment console UI kit

Everything else in this document describes the active glass direction.
...
| **MULTIMODAL**  | Claude Vision over image + question                              | rose               |
...
### Palette philosophy — light glass

The system pivots on a **cream-lavender** background (`HSL 252 33% 97%` ≈ `#F5F3FB`) and a **deep ink-violet** foreground (`HSL 245 35% 12%` ≈ `#16142A`). Major surfaces are **frosted glass panels** — translucent white at 40–70% opacity, sitting over a soft aurora bleed (lavender + peach + sky blobs that drift behind the chrome). The aesthetic is calm, atmospheric, and atypical for an internal dev tool — but never noisy: the aurora stays low-saturation, panels stay legible, type stays small.

Two brand colors carry the mark:
- **Primary — ink violet** (`HSL 250 38% 13%` ≈ `#1A1530`) — logo strokes, primary CTA, focus shadow. Quiet and authoritative; reads almost black but warmer.
- **Accent — violet** (`HSL 254 100% 61%` ≈ `#6A3AFF`) — logo dot, focus ring, score-fill, active-tab underline. Saturated, but only ever a small dose.

Semantic state colors (success / warning / destructive) are pinned to their conventional hues but **always on a 10–14% tint background with a 40% border**, never on full saturation. This keeps a dense status-heavy UI readable without screaming.

**Dark glass** (`.dark` scope): deep night `HSL 222 33% 5%`, contained aurora at low opacity, panels at 4–7% white over the bg, light-violet accent (`#B890FF`) flips in for contrast. Same vocabulary, inverted lighting model.

### Per-capability hues

The four `--cap-*` tokens drive a tiny visual moment: a 1.5 px dot in the sidebar, the icon color in the SubmitForm capability picker, and the 1 px ring + tinted border on the active card. Outside these three places, capability hues do **not** color anything else.

| Capability     | Light hex   | Dark hex    | Role                              |
|----------------|-------------|-------------|-----------------------------------|
| MOCK           | neutral ink | light slate | echo / fallback                   |
| RAG            | `#0099C4`   | `#5EE0FF`   | retrieval (cyan)                  |
| OCR            | `#6A3AFF`   | `#B890FF`   | extraction (violet, = accent)     |
| MULTIMODAL     | `#D4316F`   | `#FF8FB1`   | vision + text (rose)              |
...
- The app shell layers a **cream-lavender base** with a **soft pink radial bleed** at the top, plus two large aurora blobs (lavender + peach-to-sky) drifting behind everything via `.ds-app::before/::after`. The blobs are blurred at `110px` with `mix-blend-mode: multiply` in light, `screen` at lower opacity in dark.
- Major panels (form, sidebar, result viewer) use `rgb(var(--glass-panel))` = `white / 0.55` with `backdrop-filter: saturate(150%) blur(18px)`, a 1 px hairline border (`ink / 0.13`), and the multi-layer `--shadow-glass-panel` (white inset highlight + violet-tinted soft drop).
- Inner sub-cards (top hit, file row, OCR text, evidence, raw JSON) all use the same recipe: `glass-panel-3` (`white / 0.7`) background + `hairline-2` border + `radius 10–12px`. Density stays calm because the inner card vocabulary is consistent everywhere it appears.
- Hover lifts a panel from `panel-2` → `panel-3` and adds a 1 px translateY on tiles. No big shadows, no transforms beyond a pixel.

### Borders, shadows, radii

- **Border-radius scale:** 6 / 8 / 10 / 12 / 14 / 22 / 999 px. The default `--radius` is 10 px (buttons, inputs, sub-cards). Main glass panels use `--radius-xl` (22 px). Pill radius (`999px`) is used for status badges and the connection chip.
- **Shadows** are violet-tinted and glass-aware: every panel shadow includes a `0 1px 0 rgba(255,255,255,0.7) inset` highlight on the top edge, plus a soft `rgba(58, 30, 120, 0.18)` drop. Tokens: `--shadow-glass-panel` (main panels), `--shadow-glass-pop` (active tile, popover), `--shadow-glass-button` (primary CTA inset).
- **Borders are hairlines, not strokes** — three opacity tiers via the `--hairline` / `--hairline-2` / `--hairline-strong` tokens (8% / 13% / 22% ink). 1 px everywhere except the dropzone, which uses dashed `hairline-strong`.
...
The product ships **no photography**. The only visual assets are the SVG logo (a stylized "A" with a violet dot — `assets/logo.svg`) and the aurora blobs (CSS gradients, not images). New designs that need imagery should default to **none**; if absolutely required, prefer monochrome line illustration over photography.
...
**Brand mark:** see `assets/logo.svg`. It is a 32×32 rounded-square with a stylized capital "A" stroked in ink violet plus a 2.4 px violet dot at the lower right (`hsl(var(--accent))`). The dot **is** the brand — keep it whenever the mark is reproduced. Do not invent alternative wordmarks.
